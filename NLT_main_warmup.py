...
parser.add_argument('--e_lr', default=0.1, type=float, help='initial learning rate for encoder', dest='e_lr')
...
if args.results_dir == '':
    args.results_dir = './debug/Supervised-Warmup-' + datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
...
model = ModelBase(class_num=args.num_classes, arch=args.arch, resume_ckpt=None)
model = model.cuda()

print(args)

param_list_warmup = [
    {'params': model.encoder.parameters(), 'lr': 0.1, 'momentum': 0.9, 'weight_decay': 5e-4},
    {'params': model.classifier.parameters(), 'lr': 0.1, 'momentum': 0.9, 'weight_decay': 5e-4},
]
...
for epoch in range(100):
    ...
    test_acc_1, test_acc_per_cls, _ = test(model, test_loader, criterion, epoch, args)
    print(test_acc_per_cls)
    if test_acc_1 > best_warm_acc:
        best_warm_acc = test_acc_1
        best_warm_model = copy.deepcopy(model)
    
print('Best Warming-up Acc: {}'.format(best_warm_acc))    
