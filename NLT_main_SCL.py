...
parser.add_argument('-a', '--arch', default='ResNet32')
...
if args.results_dir == '':
    args.results_dir = './debug/Supervised-SCL-' + datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
...
def warmup(epoch, net, optimizer, dataloader, criterion):
    ...
    for imgs, labels, index in train_bar:
        ...
        train_bar.set_description('Warming-up Epoch: [{}/{}], Train Acc: {:.6f}, Loss-1: {:.4f}'.format(
                                epoch, args.epochs, 100.*correct/total, loss.item()))
...
args.resume = "/PATH/SBCL/checkpoint/cifar{}_IF{}/999.pth.tar".format(args.num_classes, int(1/imb_factor), args.arch)

saved_ckpt = torch.load(args.resume, 'cuda')
encoder = []
classifier = []
for key in list(saved_ckpt.keys()):
    if key.startswith('module.encoder') and not key.startswith('module.encoder.linear'):
        _key = key.replace('module.encoder.', '')
        encoder.append((_key, saved_ckpt[key]))

    if key.startswith('module.encoder.linear'):
        _key = key.replace('module.encoder.linear.', 'fc.')
        encoder.append((_key, saved_ckpt[key]))

    if key.startswith('module.head'):
        _key = key.replace('module.head.', '')
        classifier.append((_key, saved_ckpt[key]))

if args.arch == 'ResNet32':
    args.hidden_size = 64

model = resnet_cifar.SupConResNet(num_classes=args.num_classes, ckpt=[encoder, classifier])
model = model.cuda()
...
kkf = get_noisy_imbalanced_per_cls_list(clean_labels, args.num_classes)

prior_weights = compute_weights(0.95, get_noisy_imbalanced_per_cls_list(noisy_labels, args.num_classes))
...
for epoch in range(epoch_start, args.epochs + 1):
    ...
    train_loss, real_sample_index = train_2(model, train_loader, optimizer, criterion, scheduler, epoch, avg_prototypes, args, prior_weights)
    ...
    prior_weights = compute_weights(0.95, get_noisy_imbalanced_per_cls_list(clean_y, args.num_classes))
    prior_weights = None
