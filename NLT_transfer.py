...
if args.results_dir == '':
    args.results_dir = './debug/Supervised-Transfer-' + datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
...
def OT_consistency_2(feats, labels, prototypes, prior_weights=None):
    ...
    cos_dis = ot.dist(feats.clone().detach().cpu().numpy(), prototypes.clone().detach().cpu().numpy(), metric='cosine')
    cos_dis = torch.from_numpy(cos_dis).cuda()
    
    cos_labels4 = torch.argmin(cos_dis, -1)

    euc_dis = ot.dist(feats.clone().detach(), prototypes.clone().detach(), metric='euclidean')
    euc_labels5 = torch.argmin(euc_dis, -1)
...
res32_resume = '/PATH/Long-tailed-recognition/outputs/{}/{}_IF{}/moco_ckpt.best.pth.tar'.format(args.dataset, args.dataset, int(1/args.imb_factor))

res_32_saved_ckpt = torch.load(res32_resume, 'cuda')
res_32_encoder = []
for key in list(res_32_saved_ckpt['state_dict'].keys()):
    if key.startswith('encoder_q'):
        _key = key.replace('encoder_q.', '')
        res_32_encoder.append((_key, res_32_saved_ckpt['state_dict'][key]))

res_32_model = ModelBase(class_num=args.num_classes, arch='ResNet32', resume_ckpt=res_32_encoder)
# res_32_model = ModelBase(class_num=args.num_classes, arch='ResNet32', resume_ckpt=None)
res_32_model = res_32_model.cuda()

print(args)

res_32_param_list = [
    {'params': res_32_model.encoder.parameters(), 'lr': args.e_lr, 'momentum': 0.9, 'weight_decay': args.wd},
    {'params': res_32_model.classifier.parameters(), 'lr': args.fc_lr, 'momentum': 0.9, 'weight_decay': args.wd},
]

res_32_optimizer = torch.optim.SGD(res_32_param_list)
res_32_criterion = nn.CrossEntropyLoss(reduction='none')
res_32_scheduler = torch.optim.lr_scheduler.StepLR(res_32_optimizer, step_size=20, gamma=0.1)
...
for epoch in range(epoch_start, args.epochs + 1):
    ...
    train_fc(res_32_model, eval_loader, res_32_optimizer, criterion_fc, epoch, args)

    tmp_avg_prototypes = obtain_noised_features_prototype(model, eval_loader, 0, args)
    avg_prototypes_32 = torch.ones((args.num_classes, 64)).cuda()
    ...
    #evaluate the model    
    test_acc_1, test_acc_per_cls, test_acc_1_ot = test(model, test_loader, criterion, epoch, args, avg_prototypes)
    res_32_test_acc_1, res_32_test_acc_per_cls, res_32_test_acc_1_ot = test(res_32_model, test_loader, criterion, epoch, args, avg_prototypes_32)

    print(test_acc_per_cls)
    all_test.append(test_acc_per_cls)
    sample_per_class.append(get_noisy_imbalanced_per_cls_list(clean_y, args.num_classes))

    if res_32_test_acc_1 > best_32:
        best_32 = res_32_test_acc_1
...
print('Imbalanced Factor {}'.format(args.imb_factor))
print('Noise Ratio: {}'.format(args.noise_ratio))
print('Best Res-32 Overall Acc: {}'.format(best_32))