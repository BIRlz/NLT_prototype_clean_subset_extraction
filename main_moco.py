...
class CustomTensorDataset(Dataset):
    ...
    """
    def __init__(self, tensors, labels=None, transform=None):
    ...
    def __getitem__(self, index):
        x = self.tensors[index].astype(np.uint8)
        x = self._transform(x)
        # x = x.transpose(3, 32, 32)
        x1 = self.transform(x)
        x2 = self.transform(x)
...
loader = dataloader.cifar_dataloader('cifar10', imb_type='exp', imb_factor=0.01, noise_mode='imb', noise_ratio=0.5,\
    batch_size=256, num_workers=5, root_dir='./data/cifar-10-batches-py', log='log.txt')

# loader = dataloader.cifar_dataloader('cifar100', imb_type='exp', imb_factor=0.01, noise_mode='imb', noise_ratio=0.5,\
#     batch_size=256, num_workers=5, root_dir='./data/cifar-100-python', log='log.txt')
...
train_dataset = CustomTensorDataset(train_data, clean_labels, transform=train_transform)
memory_dataset = CustomTensorDataset(train_data, clean_labels, transform=test_transform)
...
class ModelBase(nn.Module):
    ...
    def __init__(self, feature_dim=128, arch=None, bn_splits=16):
        ...
        if arch == 'resnet18':
            ...
            net2 = resnet_arch(num_classes=feature_dim, norm_layer=norm_layer)
            ...
            self.net = resnet32(num_classes=128)
            # self.net = nn.Sequential(*self.net)
...
def test(net, memory_data_loader, test_data_loader, epoch, args):
    ...
    with torch.no_grad():
        ...
        # [N]
        feature_labels = torch.tensor(memory_data_loader.dataset.train_label, device=feature_bank.device)
